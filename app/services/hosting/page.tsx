...
export default function HostingPage() {
  return (
    <>
            ...
            {included.map((item, i) => (
              <RevealAnimation key={i} delay={(i % 2 + 1) as 1 | 2}>
                <div className="flex gap-4 p-5 h-full" style={{ border: '1px solid var(--border)', borderRadius: '8px' }}>
                  ...
                  <p className="text-sm leading-relaxed" style={{ color: 'var(--ink)' }}>{item}</p>
                </div>
              </RevealAnimation>
            ))}
          </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-16">
            ...
            <RevealAnimation delay={1}>
              <div className="p-6 rounded-lg" style={{ background: 'var(--white)', border: '1px solid var(--border)' }}>
                <p className="text-sm font-semibold text-ink mb-2">Note</p>
      ...
      <CTABand headline="Want us to host your site?" body="Get in touch and we will talk through what is involved." />
    </>
  )
}
